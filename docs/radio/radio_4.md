# 第 4 章 MMDVM (Multi-Mode Digital Voice Modem)

MMDVM != Pi-Star

说起 MMDVM 大家就会想到 Pi-Star，但是 MMDVM 不等于 Pi-Star。MMDVM 是一套独立系统，Pi-Star 是一套封装好的 MMDVM 套件(包含了 MMDVMHost , Dashboard 等软件)。

首先你需要

![](img/MMDVM.png)

事实上，我们可以单独部署 MMDVMHost，目前已知 MMDVMHost 可以运行在 Linux，Mac OS，Windows 系统上。
...
启动前确认 YSFGateway 已经启动, YSFGateway 日志中已经输出 M: 2018-11-16 14:40:58.974 Link successful to MMDVM 表示启动成功，然后再启动 MMDVMHost。
...
mmdvmhost.timer 主要的功能是延迟启动，因为 MMDVM 主板也需要初始化时间
...
Pi-Star_DV_Dash 是 pi-star 的 Dashboard，就是我们通过 IP 地址进入的界面。

项目地址是： [`github.com/AndyTaylorTweet/Pi-Star_DV_Dash`](https://github.com/AndyTaylorTweet/Pi-Star_DV_Dash)
...
### What is the difference between FCS and YSF?

[`forum.sharkrf.com/viewtopic.php?t=1750`](https://forum.sharkrf.com/viewtopic.php?t=1750)

### Trying to find non existent YSF reflector with a name of CN China 1
